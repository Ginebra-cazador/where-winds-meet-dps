...
function nodeSummary(node: OddityNode, t: (s: string) => string): string {
...
function isDefaultNode(region: string, id: number): boolean {
  return (DEFAULT_ODDITIES[region] ?? []).some((n) => n.id === id)
}
...
export function OdditiesTab({ inputs, onChange }: Props) {
  ...
  const maxNodes = useMemo(
    () => Math.max(1, ...regionKeys.map((r) => (oddities[r] ?? []).length)),
    [regionKeys, oddities],
  )
  ...
  const draw = useCallback(() => {
    ...
    const h = TOP_PAD + regionKeys.length * ROW_H
    ...
    regionKeys.forEach((region, ri) => {
      ...
      nodes.forEach((node, ni) => {
        const cx = LEFT_PAD + ni * SPACING + panX.current
        const offscreen = cx < -NODE || cx > width + NODE

        if (ni > 0) {
          const prevCx = LEFT_PAD + (ni - 1) * SPACING + panX.current
  ...
  function hitTest(px: number, py: number): NodeRef {
    for (let ri = 0; ri < regionKeys.length; ri++) {
      ...
      for (let ni = 0; ni < nodes.length; ni++) {
        ...
        if (dx + dy <= 1) return { region, id: nodes[ni].id }
      }
    }
    return null
  }
  ...
  function onMouseDown(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    function onUp(ev: MouseEvent) {
      ...
      if (drag.button === 2) {
        ...
        const node = (oddities[hit.region] ?? []).find((n) => n.id === hit.id)
  ...
  function patchNode(region: string, id: number, patch: Partial<OddityNode>) {
    const nodes = (oddities[region] ?? []).map((n) => (n.id === id ? { ...n, ...patch } : n))
    onChange({ ...inputs, oddities: { ...oddities, [region]: nodes } })
  }

  function addNode(region: string) {
    const nodes = oddities[region] ?? []
    const nextId = nodes.reduce((m, n) => Math.max(m, n.id), 0) + 1
  ...
  function removeNode(region: string, id: number) {
    const nodes = (oddities[region] ?? []).filter((n) => n.id !== id)
    onChange({ ...inputs, oddities: { ...oddities, [region]: nodes } })
  ...
  const hoveredNode = hovered
    ? (oddities[hovered.region] ?? []).find((n) => n.id === hovered.id)
    : null
  const selectedNode = selected
    ? (oddities[selected.region] ?? []).find((n) => n.id === selected.id)
    : null

  return (
    <div className="panel oddities-tab">
      <div className="cr-toolbar">
        <span className="cr-toolbar-label">{t("Oddities")}</span>
        <button type="button" className="cr-btn cr-btn-danger" onClick={resetAll}>
          {t("Reset to default")}
        </button>
      </div>

      <div className="oddities-canvas-wrap" ref={containerRef}>
        <canvas
          ref={canvasRef}
        ...
        {hoveredNode &&
          ...
          (() => {
            ...
            return (
              ...
              >
                <div className="oddity-tooltip-title">{nodeSummary(hoveredNode, t)}</div>
                <div className="oddity-tooltip-state">
...
function NodeEditor({
  ...
  return (
    <div className="panel oddity-editor">
      <h3>{t("Edit oddity node")}</h3>
      <div className="talents-cell">
        <label>{t("Stat")}</label>
      ...
      <div className="talents-cell">
        ...
        />
        <span className="talents-unit">{isRate && !isPen ? "%" : ""}</span>
      </div>
    </div>
  )
}
