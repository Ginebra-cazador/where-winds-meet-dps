...
import { GearSlotTiles } from "./GearSlotTiles"
import { GearDetailsPanel } from "./GearDetailsPanel"
import { GearInventoryPanel, type InventoryRow } from "./GearInventoryPanel"
import { GearSwapPreviewPanel } from "./GearSwapPreviewPanel"
import { NewGearPieceDialog } from "./NewGearPieceDialog"
import { RetunementAnalyzerPanel } from "./RetunementAnalyzerPanel"
import { ReattunementAnalyzerPanel } from "./ReattunementAnalyzerPanel"
...
export function GearTab({
  ...
  const liveRows: InventoryRow[] = profiles.flatMap((p) => {
    const inv = p.id === activeProfileId ? inputs.inventory : p.inputs.inventory
    const eq = p.id === activeProfileId ? equipped : p.inputs.equipped
    return inv.map((piece) => ({
      piece,
      ownerProfileId: p.id,
      ownerProfileName: p.name,
      isEquipped: eq[piece.slot] === piece.id,
    }))
  })
  const visibleRows = showGlobal
    ? liveRows
    : liveRows.filter((r) => r.ownerProfileId === activeProfileId)

  const selectedRow = selectedPieceId
    ? (liveRows.find((r) => r.piece.id === selectedPieceId) ?? null)
    : null
  ...
  function commitGearChange(nextInventory: GearPiece[], nextEquipped: EquippedSlots): void {
    ...
    for (const slot of GEAR_SLOTS) {
      ...
      const oldPiece = oldId ? (inventory.find((p) => p.id === oldId) ?? null) : null
      const newPiece = newId ? (nextInventory.find((p) => p.id === newId) ?? null) : null
  ...
  function updatePiece(updated: GearPiece): void {
    const nextInventory = inventory.map((p) => (p.id === updated.id ? updated : p))
    let nextEquipped = equipped
    const previous = inventory.find((p) => p.id === updated.id)
  ...
  async function deletePiece(): Promise<void> {
    ...
    const nextInventory = inventory.filter((p) => p.id !== selectedPiece.id)
    ...
    if (row.ownerProfileId === activeProfileId && row.piece.isNew) {
      onChange({
        ...
        inventory: inventory.map((p) =>
          p.id === row.piece.id ? (({ isNew: _drop, ...rest }) => rest)(p) : p,
        ),
      })
    }
  }
  ...
  return (
    <>
      ...
      <div className="gear-split">
        ...
        <div className="panel gear-right">
          <GearInventoryPanel
            rows={visibleRows}
      ...
      {!hideComparisons && (
        <div className="gear-split">
          <div className="gear-analyzer-stack">
