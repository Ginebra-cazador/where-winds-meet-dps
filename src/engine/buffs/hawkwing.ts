...
// ~L20653-20661) and applies the TIME-AVERAGE bonus as a flat phys multiplier
// for the whole fight (not per-frame) — the damage path uses that average,
// never the per-time `hawkwingStacksSchedule` below (display column only).
...
export function hawkwingAvgBonus(
  hitTimesSec: readonly number[],
  p: number,
  rotationDurationSec: number,
): number {
  if (hitTimesSec.length === 0 || rotationDurationSec <= 0 || p <= 0) return 0
  ...
  let total = 0
  for (let step = 0; step < steps; step++) total += accum[step] / SIM_RUNS
  const avgStacks = total / steps
  return avgStacks * BONUS_PER_STACK
}
